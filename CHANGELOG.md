...
- Doc-truth sweep: removed the archived `tweakcc-maint` consumer from the vendored-workflow references and presence matrix; corrected `changelog-health`'s fan-out count (6 → 7 enrolled repos) and folded it into the fleet's loop-count language as the fourth scheduled proposal loop; linked the previously-orphaned `staleness-setup` onboarding doc from the top-level README; `software-design` now offers `/implement` alongside `/tdd` as a terminal step, matching the `to-prd`/`to-issues`/`tdd`/`implement` chain convention; `flow-pr` now notes upstream's `code-review` rename of the installed `/review` skill so the reference stays legible until the local copy adopts it; regenerated the `installed-skills.md` snapshot against the maintainer's current global install (#520)
