...
- `harness/cli.py backlog-check` + a `Backlog throttle` step in both proposal-loop reusable bodies (`apply-agent-research`, `improve-codebase-architecture`): a read-only pre-flight count that skips the run when ≥ 3 open issues carrying the loop's own `source:` provenance label still hold `needs-triage` in the target repo — capping the untriaged *accumulation* the loops leave behind, where ADR 0019 only capped a single run's output. The threshold (`BACKLOG_THRESHOLD = 3`) and count logic live in the fetched-fresh harness so the policy reaches every consumer next run (ADR 0014); the vendored `if:` wiring only reads the `backlog_full` boolean it emits. Fails open on a count-lookup error (the per-run cap of 1 still bounds output). A full-backlog skip records a "backlog full" step summary and exits green (#543)
- `check_loop_liveness.py` + `fleet-drift.yml` (`loop-liveness` job): report-only detector flagging any recurring scheduled loop with no run within 2× its cadence, or whose last run failed — imports `check_workflow_drift.py`'s `REPOS` roster, derives cadence from each loop's cron, filters runs to `event=schedule` so a manual dispatch can't mask a dead schedule, and files a deduped per-repo `loop-liveness` issue; graceful no-op without the issues token (#532)
- `check_loop_liveness.py`: roster extended to the two host-only skills-repo loops (`changelog-health`, `skill-divergence-audit`), checked against the skills repo only — the initial #532 landing covered only the three reusable-rail loops (apply-agent-research/improve-codebase-architecture/staleness-review) across consumer repos, leaving the host-only loops unmonitored despite ADR 0037 assuming this check already covered `skill-divergence-audit` (#532)
- `skill-divergence-audit` wired as a recurring, propose-only scheduled loop: `.github/workflows/skill-divergence-audit.yml` (host-only — this repo is both tracker and catalog, so no thin-stub/reusable-body split) + `harness/prompts/skill-divergence-audit.md`; hash-staggered cron `50 2 * * 1,3,6` UTC, `claude-sonnet-5` model pin, $4.00 budget backstop — cadence and budget inherited from the `apply-agent-research` / `improve-codebase-architecture` sibling loops (ADR 0019); files at most one `source:skill-audit` issue per run through the skill's own guarded `lib/cli.py` (#522)
- ADR 0037: records the skill-divergence-audit WIRE decision (vs. delete) — binding on ADR 0019 (budget/cadence), ADR 0014 (fetched-fresh harness), and ADR 0003 (propose via issues); the official-catalog second divergence axis (#514) stays explicitly deferred (#522)
- `skill-divergence-audit/lib/divergence.py`: Pass 2 now excludes `CONTEXT.md`'s soft-dependency `mattpocock/skills` skills (`SOFT_DEPENDENCY_SKILLS`, ~11 names) from `MISSING_HERE` — that gap is the intended soft-depend posture (ADR 0024), not a finding; `lib/cli.py`'s `gate` help text corrected `<=5` → `<=1` to match the enforced per-run cap (third instance of the stale-numeral class, after #375, #485) (#522)
...
- Proposal loops now apply triage labels at filing time (#668): the two harness-`publish` loops (`improve-codebase-architecture`, `staleness-review`) pass `--extra-label needs-triage --extra-label chore` plus `--dedup-open` (single-open-advisory — the dedup rider #668 asked for, at zero new machinery). `apply-agent-research` files through its own `skills/meta/apply-agent-research/lib/cli.py file` (already repeat-`--label` capable, with its own per-capability dedup), so its half is a prompt change adding `needs-triage` + a category to the **self-improvement** channel only (`skill-request`/`skill-promotion` keep their own drain), with the reusable workflow's label-ensure step now pre-creating `needs-triage`/`enhancement`/`chore` (best-effort, alongside `source:agent-research`) so that prompt-only path can't hard-fail on a missing label either. `harness/cli.py publish` gains a repeatable `--extra-label`, threaded into `_create_issue` and each **ensured** (`gh label create`, idempotent) before the create so an unknown label in a target repo can't hard-fail a filing. Closes the `source:*`-only issues the loops were leaving invisible to state-filtered triage. Ships by moving the mutable `claude-loops-v1` tag; agent-research (SHA-pinned) is unaffected until separately bumped — the change is additive/backward-compatible (#668)
- `flow-pr` review→fix loop: a `/code-review` finding that survives the FP-exclusion list is now stated to be confirmed actionable — the fix pass returns the specific fix or the concrete failure point it hits, and "the original is fine" is no longer available at that stage (that judgment already happened in the FP-exclusion pass). Closes a relitigation loophole where the fixer could re-open a decided finding and stall the loop (crib V7, spike #558)
- `docs/onboarding/proposal-loop-harness.md`: corrected the `fetch-rubric` reference to match the shipped CLI (`--source-dir` required, network fallback gone — doc-truth debt from #561, which updated the code but not this page), and added a **lane-aware tag-move rollout order** to the rollout section — a tag move reaches only tag-pinned consumers, so SHA-pinned consumers (agent-research) must be bumped before any fetched-fresh-harness compatibility shim is deleted, or the SHA-pin lane breaks (the 2026-07-04 #561/agent-research#491 incident) (#562)
- `autonomous-loop` (`SKILL.md` element 2 + `RUNNING-AFK.md`): the per-iteration gate now prescribes context-efficient output — a single deterministic marker per check on success, full output reserved for the failing check, so a clean run across many unattended items doesn't itself become a context-rot source (#511)
...
- `flow-pr`'s review gate is repointed from `/review` to `/code-review` — the installed two-axis Standards+Spec skill that resolves for that name today; all six `SKILL.md` references plus the disambiguation blockquote and the `README.md` catalog line updated. Supersedes the earlier stay-unchanged fallback note; the documentary-pin + gated-sync apparatus was deliberately skipped as speculative (#556, was #520)
- `frontend-design` skill compressed ~40 lines (824 → 783): cross-file OKLCH/`light-dark()` and "no raw values" restatements consolidated to one authority (`design-tokens.md`) plus pointers, and the direction-doc worked example and Motion/shadow tutorials tightened to spec. All rules and the four differentiated layers (#528) are intact; the KEEP verdict is unaffected (#557)
...
- `skill-divergence-audit` (`lib/divergence.py`): `render_report` sorted findings alphabetically by category, contradicting `SKILL.md`'s documented priority order (DIVERGED → MISSING_HERE → OUTDATED_HERE → NO_UPSTREAM_EQUIVALENT) — alphabetical placed `NO_UPSTREAM_EQUIVALENT` ahead of `OUTDATED_HERE`; a `CATEGORY_ORDER` map now enforces the documented order, name still breaking ties (#541)
- `setup-dividedby-skills` (`SKILL.md` Concern G1): replaced the undefined-as-used "de-slop F3" reference with "a partial de-slop pass" (#510)
