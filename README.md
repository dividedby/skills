...
- **[doc-regen](./skills/engineering/doc-regen/SKILL.md)** — Reconcile a repo's roadmap census with live GitHub issue state so the doc stays trustworthy as the single source for picking the next task: bootstrap the pattern, migrate a legacy planning doc, or repair drift in three tiers. Edits the working tree for review and writes additive issue comments; never commits or closes.
