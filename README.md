...
- **[flow-pr](./skills/engineering/flow-pr/SKILL.md)** — End-to-end flow-aware PR helper: detects repo role, cuts a feature branch, commits → pushes → opens PR with the right base, gates on CI green, then merges to the integration branch. Promotion (staging→main) is always-confirmed.
