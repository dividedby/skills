...
description: >
  ...
  proposes realignment issues — at most five per run, each cleared against a
...
issues — labeled `source:skill-audit`, capped at five per run, deduplicated
...
The **≤5-per-run cap**, **dedup**, and **leak guard** are enforced by code,
...
each carrying one of four categories:

| Category | Meaning |
|---|---|
| `MISSING_HERE` | Upstream names a skill/practice we have no equivalent for |
| `OUTDATED_HERE` | We have a skill for this surface but are missing pillar(s) upstream covers |
| `DIVERGED` | We cover the same surface but our guidance directly contradicts upstream on a named point |
| `NO_UPSTREAM_EQUIVALENT` | We have a skill with no upstream analogue — worth confirming as intentional |

`ALIGNED` (same surface, no gap) is computed but not proposed; it appears
only in a verbose dump. The classifier lives in pure code so the categorization
is reproducible and not a per-run judgment call.
...
The gate returns at most **five** ranked candidates. For each, write the issue
...
- **Filing ALIGNED or NO_UPSTREAM_EQUIVALENT findings.** Aligned means no gap.
  `NO_UPSTREAM_EQUIVALENT` is an observation, not an action item — surface it
  in the report for human review, do not propose it.
...
- **Filing more than five proposals.** The budget is a ceiling and the code
  enforces it. A typical run files 0–2; filing 5 means 5 independently strong
  proposals.
