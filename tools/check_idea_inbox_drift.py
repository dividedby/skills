...
def ensure_label(write_token: str, dry_run: bool) -> None:
    """Best-effort: create the idea-inbox-drift label in skills if absent."""
    if dry_run:
...
def open_issues_for_idea_inbox_drift(write_token: str) -> set[str]:
    """Return the set of open issue titles tagged idea-inbox-drift in skills."""
    result = _gh(
        [
            "issue", "list",
            "--repo", SKILLS_REPO_WRITE,
...
def main(argv=None):
    ...
    if args.dry_run:
        ...
        open_titles = open_issues_for_idea_inbox_drift(write_token)
