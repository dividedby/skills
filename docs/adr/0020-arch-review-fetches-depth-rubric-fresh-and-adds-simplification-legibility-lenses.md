...
> **Second amendment, 2026-06-20.** Re-sync against upstream's next change: `improve-codebase-architecture` now generates a self-contained interactive HTML report (Tailwind + Mermaid via CDN, written to a tmpdir and opened in a browser), and `codebase-design` gained `DESIGN-IT-TWICE.md` (an interactive parallel-design technique that spawns competing-constraint sub-agents for a live user to deliberate). **The two pinned rubric URLs (`codebase-design/{SKILL,DEEPENING}.md`) still resolve — no harness change forced.** Both new artifacts are interactive-maintainer flows, structurally incompatible with this headless propose-only loop, so per [ADR 0024](0024-lean-on-upstream-skills-soft-depend-over-reinvent.md) neither is fetched, pinned, or reimplemented here — they stay a soft-depend for user-invoked runs. **One adaptation is absorbed:** the *principle* of "design it twice" is added async to the skeleton's body-drafting rules (Step 5) as a **Design-tension** block — each proposal names 2–3 candidate-specific competing constraints, sketches the design under each, and states the decision the human must resolve at triage. This is inlined in the skeleton (not fetched: `DESIGN-IT-TWICE.md` is interactive-only and the principle is short — same rationale as Decision (d)'s Simplification lens), adds no fetch URL and no sub-agents. Decision (a)'s pins and the existing "one recommendation, not a menu" rule are unchanged; the Design-tension section is subordinate decision-support, not a second recommendation.
