...
1. **No single upstream covers the capability.** 0024's default triggers when an upstream skill "covers the capability." None does — the value is the *synthesis* of narrative structure (F1) with de-slop/voice (F3), English-only. ADR 0024 itself classifies differentiated synthesis as the independent value this repo keeps.
